...
UNLOCK_TEXT <- list(
  "0" = list(
    ...
    actor = "A person",
    waits_on = "Nothing. No machine is rented and no gated file is fetched before the approvals are held, which is why the cost of being blocked here is zero."),
  "1" = list(
    action = "Fetch the file defining which cases are scored",
    actor = "Machine",
    waits_on = "The access request has been granted, and the fetch is made by the rented machine under its own credential."),
  "2" = list(
    action = "Check the file before spending machine time",
    actor = "Machine",
  ...
  "3" = list(
    action = "Obtain the images the file names",
    actor = "A person, then machine",
    waits_on = "Each image source's own agreement is held. Which agreements those are is decided by the file, not by the project."),
  "4" = list(
    action = "Generate a report for every case",
    actor = "Machine",
    waits_on = "The three preceding steps have passed and the model is already on the machine."),
  "5" = list(
