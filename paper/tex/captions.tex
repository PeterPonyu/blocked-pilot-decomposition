...
counts from the \FullSplitN{}-row frozen test split; filled blue points are positive counts
in the pre-registered \NCohort{}-row draw, and open points are expected from full-split
prevalence. The horizontal rule is the \MinPresent{}-positive presence gate: cells with
\(n_{\mathrm{present}}<\MinPresent{}\) are coloured red and are underpowered, so no rate is
attached. The observed points come from the registered draw, while the frozen test split
supplies the full-split bars. This is labeller-output prevalence rather than clinical prevalence;
the compact x-axis key is: \FindingKey.%
}
