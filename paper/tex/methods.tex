...
Labels are the fourteen observations of the CheXpert schema~\cite{irvin2019chexpert},
...
The checkpoint used is recorded with its SHA-256 digest in the bound artifact, so
...
rows. The retrieval control embeds the indication field with TF--IDF, retrieves
...
Three things this project could have produced and did not, each because
producing it would have made the result look stronger than the access situation
allows.

No score is reported under the blocked benchmark's name. The locally held corpus
is a different corpus, and reporting a number on it under the benchmark's label
would be a category error rather than an approximation. The project's own scoring
wrapper enforces this by refusing any sample that is not the benchmark's split
size.

No standard automatic report-generation metric is computed from these labels.
The label counts here support statements about positives and misses; converting
them into the field's headline aggregate would invite comparison against
published values computed on a different corpus with a different sample size.

No inference run is promoted past what it is. A local run of a chest-radiograph
foundation model over \HarnessImages{} images exists and is recorded, and it is
recorded as a harness that checks the pipeline executes, with its tier marked as
not the official evaluation. It contributes no number to this paper.

\subsection{Evidence discipline}

No number in this manuscript is typed by hand. Every artifact it rests on is
recorded with its SHA-256 digest in a manifest that accompanies the manuscript
source; the figure and table code verifies each digest against the bytes on disk
