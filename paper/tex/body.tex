...
written on \BlockerDate{}, before computation was provisioned. The alternative --- to
provision compute, discover the gate, and write the discovery up as a cost --- is
...
Fig.~\ref{fig:reasons} places those three owners on one page. It adds no scored
cell and no new lock. It exists so the rest of the paper can be read as three
records rather than as one vanished experiment.
...
significance~\cite{button2013power,ioannidis2005why,openscience2015repro}. Flexibility in analysis and selective reporting make the
same small samples look more decisive than they
are~\cite{simmons2011false,gelman2014garden,head2015phacking,amrhein2019retire}.
That argument is usually deployed against studies that reported a result anyway.
The situation here is the one that argument implies but rarely addresses
directly: a pilot whose subsample is too small for most of the label schema,
where the honest output is a per-cell statement of what could and could not be
scored. Separating the two power questions --- whether the corpus could ever
support a cell, and whether the particular draw resembled the corpus --- is what
keeps a corpus-level ceiling from being misread as a sampling accident, and
neither question is answered by reporting the cells that happened to clear a
gate. Pre-registration prevents choosing the sample after seeing its
labels~\cite{nosek2018preregistration,munafo2017manifesto}; it does not enlarge
the corpus. The intervals we do report on scorable cells are the ordinary
binomial and bootstrap constructions~\cite{clopper1934use,efron1979bootstrap},
and the paired comparisons that appear later use the sign and signed-rank
...
oakdenrayner2020hidden,obermeyer2019dissecting,nazer2023bias}. What is missing is not awareness of the
barriers but any convention for reporting a specific encounter with one ---
which permissions were held, which were not, what each would and would not
unlock, and what the attempt cost. This paper is an attempt to write that
report for one case, and to do it in a form that can be checked rather than
asserted.
...
access''. None of those three requests was filed. The permission locks record
that absence, not a denial.
...
project is recorded as a smoke test that checks the code runs end to end and
...
be shortened from the other end, and the steps that an authorized runner could
...
the benchmark, written down before computation was provisioned. Step is the recorded
order; a step cannot begin until every step above it has finished. Who acts
distinguishes the steps that need a person from those an authorized runner performs
...
below the presence gate (Fig.~\ref{fig:prevalence-sampling-ceiling}). The obvious reading is that the pilot is too small. For
...
evidence, and the positive counts that drive the gate are only one component of
the complete label partition. This is a description of the labeller's output,
not a claim about clinical prevalence.
...
The assignment is not a property of the whole template taken as one
string. A battery of \BatteryN{} controlled variants isolates what the
released checkpoint responds to. The raw template is marked positive for
\BatteryRawPositive{} and is not marked No Finding. Deleting the first
sentence, which is the only clause that contains the de-identification
placeholder, flips the same template to \BatteryMinusFirst{}.
\BatteryPlausibleUncertain{} plausible English wordings of that
placeholder---``remained stable'', ``not changed'', ``no change'',
...
Of the gold texts, \GoldPlaceholderTexts{} carry the placeholder.
Relabelling those texts with the placeholder removed changes any label
on \GoldRowsChanged{} rows and moves no finding's present count by more
than \GoldMaxDeltaPresent{}, while individual label states move up to
\GoldLungOpacityAnyState{} times for Lung Opacity.
...
A later, post-hoc scoring of the same controls on the frozen
\FullSplitN{}-report split, after the registered analysis was frozen, finds
that \FullSplitScorable{} of the \NFindings{} findings clear the presence
gate of \MinPresent{} gold positives, and the \NCohort{}-row registered draw
...
asserts Cardiomegaly on all \FullSplitMajCardioPred{} rows. The
\NCohort{}-row draw remains the registered analysis, including the count that
the labeller marks the constant string \MajAsserted{} on all \NCohort{}
rows; that count is unchanged. The \FullSplitN{}-row scoring is a post-hoc
...
we have recorded the checkpoint checksum precisely so that can be attempted.
...
same project is recorded as a smoke test and contributes no number.
...
computation was provisioned rather than after. Of \NFindings{} labelled observations,
...
and that every artifact behind one is pinned by checksum. Table~\ref{tab:evidence}
is that assertion in a form a reader can check. It lists all \NEvidence{} recorded
...
without them invites more confidence than it earns. First, a checksum binds bytes
...
The checksums are also what makes the negative results in this paper durable. A
...
Recording the obstacle enumeration and the blocked evaluation with their checksums, before any
...
hexadecimal characters of its SHA-256 checksum. Full checksums, and the paths the
checksums apply to, are in the manifest that accompanies the manuscript source;
the prefix is shown here only because a 64-character checksum does not set in a
table column. The figure and table code recomputes each checksum from the bytes on
...
release identified in the manifest by checksum rather than copied.
...
including the fixed majority control, the presence gate, and agreement between
recorded and recomputed thin-observation lists --- are enforced by build-time
